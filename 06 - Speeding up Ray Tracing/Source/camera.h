...
public:
    ...
    void render(const hittable& world) {
        ...
        if (window)
        {
            ...
            int lXOffset = 0;
            int lYOffset = 0;
            int uXOffset = image_width - windowWidth * (numThreads - 1);
            int uYOffset = image_height - windowHeight * (numThreads - 1);

            for (int x = 0; x < numThreads; x++)
            {
                lYOffset = 0;
                uYOffset = image_height - windowHeight * (numThreads - 1);
                for (int y = 0; y < numThreads; y++) {

                    RayTracingThread* thread = new RayTracingThread(id, 
																this,
																	rtImage,
																	lXOffset,
																	lYOffset,
                    ...
                    lYOffset = uYOffset;
                    uYOffset += windowHeight;
                    id++;
                }

                lXOffset = uXOffset;
                uXOffset += windowWidth;
            }
		}
        else
        {
            for (int y = 0; y < image_height; y++) {
                ...
                RayTracingThread* thread = new RayTracingThread(y, this, rtImage, 0, y, image_width, y + 1, world);
